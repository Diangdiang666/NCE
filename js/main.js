...
// 保留原作者注释
// Author: Qim | Integrated with ASDQ by Gemini
...
class ReadingSystem {
  constructor() {
    this.state = {
      // ... 保留原有 state
      books: [], units: [], bookPath: '', bookKey: '',
      currentLyrics: [], currentLyricIndex: -1, currentUnitIndex: -1,
      playMode: 'single', playbackRate: 1.0, translationMode: 'show',
      reciteMode: false, availableSpeeds: [0.5, 0.75, 1.0, 1.25, 1.5, 2.0],
      savedPlayTime: 0, isProgressDragging: false,
      // 新增 state
      autoPause: false,
      singlePlayEndTime: null
    };
    ...
    this.dom = {
      // ... 原有 dom
      audioPlayer: qs('#audioPlayer'), lyricsDisplay: qs('#lyricsDisplay'),
      lyricsContainer: qs('.lyrics-container'), bookName: qs('#bookName'),
      bookLevel: qs('#bookLevel'), unitList: qs('#unitListContainer'),
      playModeBtn: qs('#playModeBtn'), playPauseBtn: qs('#playPauseBtn'),
      progressBar: qs('#progressBar'), speedBtn: qs('#speedBtn'),
      speedText: qs('#speedText'), bookCover: qs('#bookCover'),
      unitSelect: qs('#unitSelect'), bookSelects: qsa('.book-select'),
      prevUnitBtn: qs('#prevUnitBtn'), nextUnitBtn: qs('#nextUnitBtn'),
      toggleTranslationBtn: qs('#toggleTranslationBtn'), reciteModeBtn: qs('#reciteModeBtn'),
      // 新增 dom
  ...
  async init() {
    ...
    await this.applyBookFromHash();
    this.bindEvents();
    this.loadPreferences(); // 合并原有的 preference 加载
    await this.loadUnitFromStorage();
  }
  ...
  bindShortcuts() {
    document.addEventListener('keydown', (e) => {
      ...
      switch(key) {
        case 'a': this.jumpSentence(-1); break;
        case 'd': this.jumpSentence(1); break;
        case 's': this.jumpSentence(0); break;
        case 'q': if (this.dom.autoPauseBtn) this.dom.autoPauseBtn.click(); break;
        case ' ': e.preventDefault(); this.dom.playPauseBtn.click(); break;
      }
    });
  }
  ...
  jumpSentence(offset) {
    ...
    if (targetIdx >= 0 && targetIdx < this.state.currentLyrics.length) {
      ...
      this.playLyricAtIndex(targetIdx, lyric.time);
      // 立即反馈变色，不等待 audio 缓冲
      this.updateUIHighlight(targetIdx);
    }
  }
  ...
  bindEvents() {
    this.bindShortcuts();
    this.bindReciteToggle(); // 原有的 Ctrl/点击背诵功能
    ...
    this.dom.audioPlayer.ontimeupdate = () => {
      this.updateLyricHighlight(); // 原有的高亮
      this.checkAutoPauseLogic();  // 新增的自动暂停
      this.checkSinglePlayEnd();
      this.updateProgress();
    };

    // ... 其他原有的 audioPlayer 事件 (onplay, onpause, onended) 保持不变 ...
    this.dom.audioPlayer.onplay = () => this.updatePlayButton();
    this.dom.audioPlayer.onpause = () => this.updatePlayButton();
    this.dom.audioPlayer.onended = () => { if(this.state.playMode === 'continuous') this.jumpSentence(1); };

    // ... 原有的 unitList, unitSelect, bookSelect 监听保持不变 ...
  ...
  // ==== 以下所有函数（loadBooks, loadUnitByIndex, initThemeToggle 等）均保持原作者代码逻辑 ====
  // (此处省略重复的原版函数代码，实际部署时请确保它们完整存在)
  ...
  // ... 此处粘贴你原代码中剩余的：injectReciteStyles, loadBooks, applyBookChange, 
  // renderLyrics, updateLyricHighlight, cyclePlaybackSpeed 等所有函数 ...
}

// 保持原有的全局初始化函数
document.addEventListener('DOMContentLoaded', () => {
  new ReadingSystem();
  initThemeToggle(); // 确保原有的黑夜模式开关运行
  initSupportModal();
});

// ... 保持原有的 initThemeToggle, initSupportModal, LRCParser 类 ...
